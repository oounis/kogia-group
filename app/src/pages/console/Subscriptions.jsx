...
import { PageHead, Badge, Table, Empty, Avatar, Btn } from '../../ui.jsx'
      ...
      <Table head={['Client', 'Produit · Plan', 'Sièges', 'Prochaine échéance', 'Montant', 'Renouvellement', '']}>
        {list.map(s => (
          <tr key={s.id} className="hover:bg-canvas">
            <td className="px-4 py-3"><div className="flex items-center gap-2.5"><Avatar name={s.client?.name} color={s.client?.color} size={30} /><span className="font-semibold">{s.client?.name}</span></div></td>
        ...
        {!list.length && <tr><td colSpan={7}><Empty>Aucun abonnement</Empty></td></tr>}
      </Table>
    </div>
  )
}
